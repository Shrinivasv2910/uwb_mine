function out = m4_run_trust(cfg, net, path, seed, policy)
% M4_RUN_TRUST  Fly a fixed path under one measurement-trust policy.
%   policy: 'fixed' (R=R0 always), 'hard' (drop anchor if P>=0.5),
%           'soft' (R = R0*(1+lambda*P), clamped).
% Detector and path identical across policies; only the trust rule changes.
...
for k=1:N
    ...
    for i=1:nA
        switch policy
            ...
            case 'hard',  Reff(i)=Rmeas; useIt(i)=pHat(k,i)<pThr;
            case 'soft'
                Reff(i)=min(Rmeas*(1+cfg.softLambda*pHat(k,i)),cfg.softRmax);
                useIt(i)=pHat(k,i)<cfg.pGateHard;
            otherwise, error('unknown policy %s',policy);
        end
    end
    if nnz(useIt) < 3                       % GUARD: keep 3 least-suspect anchors
...
out=struct('rmse',sqrt(mean(err.^2)),'err',err,'exposure',mean(pf(:)));
end
